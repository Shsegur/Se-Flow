...
export default function App() {
  ...
  const handleDownloadFlyer4K = async () => {
    try {
      ...
      if (!storyNode) {
        showNotify('No se encontró la vista previa del flyer', 'error');
        return;
      }
      ...
      const sourceCanvas = await html2canvas(storyNode, {
    ...
    <div className="space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-700 no-print">
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3 sm:gap-4">
        ...
        <Card onClick={() => navigateToCobros('sin_informar')} className="bg-orange-50 dark:bg-orange-900/10 border-orange-100 dark:border-orange-900/30 group flex flex-col justify-between">
          <div className="flex items-center justify-between mb-2"><p className="text-[9px] sm:text-[10px] font-black text-orange-600 uppercase tracking-widest leading-tight">Cuotas<br/>Sin Informar Pago</p><Clock size={16} className="text-orange-500/40" /></div>
         ...
         <Card className="w-full max-w-md p-8 sm:p-10 text-center border-t-4 border-t-blue-600">
            ...
            <form onSubmit={handleLogin} className="space-y-4 text-left">
               ...
               {!isInstalled && !canInstall && (
                 <button type="button" onClick={() => setIsInstallHelpOpen(true)} className="w-full text-[10px] text-slate-400 text-center underline underline-offset-2">
                   ¿Cómo instalar?
                 </button>
               )}
            </form>
      ...
      {isInstallHelpOpen && !isInstalled && (
        <div className="fixed inset-0 z-[221] flex items-center justify-center p-4 no-print">
          ...
          <div className="relative bg-white dark:bg-slate-900 w-full max-w-md rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-700 p-6 sm:p-7">
            ...
            <p className="mt-4 text-[11px] text-slate-400">Si no aparece la opción, recarga la página y vuelve a intentar.</p>
