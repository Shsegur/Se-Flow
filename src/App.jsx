...
export default function App() {
  ...
  const handleInstallApp = async () => {
    ...
    if (!promptEvent) {
      ...
      setInstallHintMessage(
        isDesktopChrome
          ? 'Chrome aún no habilitó el prompt automático. En Mac: barra de direcciones > icono de instalar, o menú ⋮ > Instalar Se-Flow.'
            ...
            <form onSubmit={handleLogin} className="space-y-4 text-left">
               ...
               {!isInstalled && (
                 <button type="button" onClick={handleInstallApp} disabled={isInstalling} className="w-full py-3 bg-emerald-600 text-white font-black rounded-2xl uppercase tracking-widest text-[10px] shadow-lg shadow-emerald-500/20 hover:-translate-y-1 transition-all disabled:opacity-50">
                   {isInstalling ? 'Instalando...' : (canInstall ? 'Instalar' : 'Instalar (Guía)')}
                 </button>
               )}
               {!isInstalled && (
      ...
      {isInstallHelpOpen && !isInstalled && (
        <div className="fixed inset-0 z-[221] flex items-center justify-center p-4 no-print">
          ...
          <div className="relative bg-white dark:bg-slate-900 w-full max-w-md rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-700 p-6 sm:p-7">
            ...
            <p className="mt-4 text-[11px] text-slate-400">Si no aparece la opción, recarga la página en Chrome/Safari normal y vuelve a intentar.</p>
