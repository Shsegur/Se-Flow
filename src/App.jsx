...
export default function App() {
  ...
  const validUsers = [
    { id: 'u1', user: 'admin', pass: '1234', role: 'admin', name: 'Administrador General' },
    { id: 'u2', user: 'atencion', pass: '1234', role: 'agent', name: 'Atención al Público' }
  ];
  ...
  const [agencySettings, setAgencySettings] = useState({
    name: 'AseguraPro Central', logo: '', phone: '', address: '', installmentNotifyMode: 'manual', policyNotifyMode: 'manual', notifyHour: '09:00', reminderDays: [3, 1, 0] 
  });
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(auth, (u) => {
      ...
      if (u) {
        ...
        onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'installments'), (snap) => {
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    const found = validUsers.find(u => u.user === loginForm.user && u.pass === loginForm.pass);
    if (found) { setAppUser(found); setLoginError(''); logAction('Inicio Sesión', 'Acceso al sistema', found); } 
  ...
  const handleImageUpload = async (e) => {
    ...
    if (!file) return;
    if (!storage) {
      showNotify('Configura Firebase Storage para subir imágenes', 'error');
      return;
    }

    try {
      const safeName = `${Date.now()}_${file.name.replace(/\s+/g, '_')}`;
      const storageRef = ref(storage, `artifacts/${appId}/policy-photos/${safeName}`);
      await uploadBytes(storageRef, file, { contentType: file.type || 'image/jpeg' });
      const downloadUrl = await getDownloadURL(storageRef);
      setPolicyPhotos(prev => [...prev, downloadUrl]);
      showNotify('Imagen subida a Firebase Storage');
    } catch (error) {
      console.error(error);
  ...
  const renderSettings = () => {
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-2 animate-in fade-in duration-500 no-print">
        ...
        <AccordionItem 
          title="Identidad Visual" icon={Palette} isOpen={openSettingsTab === 'identidad'} 
          onToggle={() => setOpenSettingsTab(openSettingsTab === 'identidad' ? null : 'identidad')}
          onSave={() => { setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'settings', 'agency'), agencySettings); logAction('Ajustes', 'Identidad Visual'); showNotify('Identidad guardada'); }}
        >
           <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-2">Logo URL (PNG)</label><input value={agencySettings.logo} onChange={e=>setAgencySettings({...agencySettings, logo: e.target.value})} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 rounded-xl outline-none font-medium text-xs dark:text-white border border-slate-100 dark:border-slate-700" placeholder="https://..." /></div>
           </div>
        </AccordionItem>
...
VITE_APP_ID=se-flow`}
    ...
    return (
      <div className="min-h-screen bg-[#FDFDFD] dark:bg-slate-950 flex items-center justify-center p-4">
         <Card className="w-full max-w-md p-8 sm:p-10 text-center border-t-4 border-t-blue-600">
            ...
            <h2 className="text-2xl font-black text-slate-800 dark:text-white uppercase tracking-tighter mb-1">AseguraPro</h2>
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] dark:bg-slate-950 flex font-sans text-slate-900 dark:text-slate-100 overflow-x-hidden transition-colors duration-500 w-full">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-[80%] max-w-[280px] lg:max-w-none lg:w-72 bg-white dark:bg-slate-900 border-r border-slate-50 dark:border-slate-800 transform transition-transform duration-500 lg:relative lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} no-print flex flex-col`}>
        <div className="p-6 lg:p-8 flex flex-col h-full overflow-y-auto scrollbar-hide">
          <div className="flex items-center gap-3 sm:gap-4 mb-8">
            <div className="w-8 h-8 sm:w-10 sm:h-10 shrink-0 bg-slate-900 dark:bg-blue-600 rounded-lg sm:rounded-xl flex items-center justify-center shadow-lg"><ShieldCheck className="text-white" size={18} /></div>
            <div className="leading-tight"><h1 className="text-base sm:text-lg font-semibold tracking-tighter uppercase dark:text-white">Asegura<span className="text-slate-300 font-light">PRO</span></h1><span className="text-[7px] sm:text-[8px] font-bold text-slate-400 uppercase tracking-[0.4em]">Manager v12</span></div>
          </div>
          ...
          <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-2xl mb-8 flex items-center gap-3 border border-slate-100 dark:border-slate-700/50">
             ...
             <div className="overflow-hidden"><p className="text-[10px] font-black uppercase text-slate-800 dark:text-white truncate">{appUser.name}</p><p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest truncate">Rol: {appUser.role}</p></div>
          </div>

          <nav className="flex-1 space-y-1 sm:space-y-2">
            {[ { id: 'dashboard', icon: LayoutDashboard, label: 'Inicio' }, { id: 'clients', icon: Users, label: 'Clientes' }, { id: 'vencimientos', icon: CalendarDays, label: 'Cobros' }, { id: 'settings', icon: Settings, label: 'Ajustes' } ].map(item => (
      ...
      <div className="flex-1 flex flex-col h-screen relative no-print w-full max-w-full overflow-x-hidden">
        <header className="h-16 sm:h-20 bg-white/50 dark:bg-slate-950/50 backdrop-blur-3xl border-b border-slate-50 dark:border-slate-900 flex items-center justify-between px-4 sm:px-6 lg:px-10 sticky top-0 z-30">
          <div className="flex items-center gap-3 sm:gap-4"><button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 sm:p-2.5 text-slate-500 bg-slate-50 dark:bg-slate-900 rounded-lg sm:rounded-xl"><Menu size={18} /></button><h2 className="text-sm sm:text-md font-semibold text-slate-800 dark:text-white tracking-tight capitalize">{activeTab}</h2></div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-10 scroll-smooth w-full">
          {loading ? ( <div className="h-full flex flex-col items-center justify-center gap-3 sm:gap-4"><div className="w-8 h-8 sm:w-10 sm:h-10 border-2 border-slate-900 dark:border-blue-600 border-t-transparent rounded-full animate-spin"></div><p className="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-widest">Sincronizando Sistema</p></div> ) : (
            <>
      ...
      {isBroadcastOpen && (
        <div className="fixed inset-0 z-[130] flex items-center justify-center p-2 sm:p-4 overflow-hidden no-print">
          ...
          <div className="relative bg-[#FDFDFD] dark:bg-slate-900 w-full max-w-6xl rounded-[2rem] sm:rounded-[4rem] shadow-2xl overflow-hidden animate-in zoom-in slide-in-from-bottom-8 duration-500 flex flex-col lg:flex-row h-[95vh] max-h-[95vh]">
             <div className="p-4 sm:p-8 lg:p-10 flex-1 overflow-y-auto bg-white dark:bg-slate-900 scrollbar-hide lg:border-r border-slate-100 dark:border-slate-800">
                <div className="flex items-center justify-between mb-6 sm:mb-8">
                   <div>
                     <h3 className="text-lg sm:text-2xl font-semibold dark:text-white tracking-tight flex items-center gap-2"><Palette className="text-blue-500" size={20} /> Asegura Studio</h3>
                ...
                <div className="space-y-6 sm:space-y-8">
                   <div>
                      <label className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 sm:mb-4 block">Estilos Predefinidos</label>
                      <div className="flex gap-3 sm:gap-4 overflow-x-auto pb-2 sm:pb-4 scrollbar-hide">
                         {designPresets.map((p, i) => (
                            <button key={i} onClick={() => handleApplyPreset(i)} className={`shrink-0 flex flex-col items-center gap-1 sm:gap-2 group`}>
                               <div style={{ background: p.bg }} className={`w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-3xl border-2 transition-all ${selectedPreset === i ? 'border-blue-500 scale-110 shadow-xl shadow-blue-500/20' : 'border-transparent opacity-70 group-hover:opacity-100'}`} />
                   ...
                   <div className="space-y-4 sm:space-y-6">
                      <div className="p-4 sm:p-5 bg-slate-50 dark:bg-slate-800/50 rounded-2xl sm:rounded-3xl space-y-3 sm:space-y-4 border border-slate-100 dark:border-slate-700/50">
                         ...
                         <div className="border border-slate-200 dark:border-slate-700 rounded-xl sm:rounded-2xl overflow-hidden bg-white dark:bg-slate-800">
                            ...
                            <div 
                               ...
                               className="w-full h-24 sm:h-32 p-3 sm:p-4 outline-none text-xs sm:text-sm editable-container text-slate-800 dark:text-slate-200 overflow-y-auto"
                   ...
                   <div className="p-4 sm:p-5 bg-slate-50 dark:bg-slate-800/50 rounded-2xl sm:rounded-3xl border border-slate-100 dark:border-slate-700/50 space-y-3 sm:space-y-4">
                      ...
                      <input 
                         ...
                         className="w-full px-3 py-2 sm:px-4 sm:py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg sm:rounded-xl outline-none focus:ring-2 focus:ring-blue-500/20 dark:text-white text-[10px] sm:text-xs transition-all" 
                      />
                   </div>
