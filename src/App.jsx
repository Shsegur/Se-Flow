...
export default function App() {
  ...
  const dashMetrics = useMemo(() => {
    ...
    const vencidas = polNoBaja.filter(p => p.vigenciaHasta < todayStr).length;
  ...
  }, [policies, processedInstallments, todayStr, pendingPaymentClients.length]);
    ...
    <div className="space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-700 no-print">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 sm:gap-8">
          ...
          <div className="space-y-4">
            {reminderDays.map((day) => (
              <button
                ...
                className="w-full text-left rounded-2xl border border-slate-100 dark:border-slate-700/50 p-3 hover:border-blue-200 hover:bg-blue-50/30 dark:hover:bg-slate-900/40 transition-colors"
              >
                <div className="flex items-center justify-between gap-2 mb-2">
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{reminderLabel(day)} · {installmentReminderGroups[day].length}</p>
                  <span className="px-2.5 py-1.5 rounded-lg bg-blue-50 text-blue-600 text-[8px] font-black uppercase tracking-widest">
                    Ver listado
                  </span>
                </div>
                <div className="space-y-2">
                  {installmentReminderGroups[day].slice(0, 4).map(({ inst, client }) => (
                    <div key={inst.id} className="flex items-center justify-between gap-2 p-2 bg-slate-50 dark:bg-slate-900/50 rounded-xl">
                      <div className="truncate">
                        <p className="text-[10px] font-black text-slate-800 dark:text-white truncate">{client.name} · {inst.matricula}</p>
                        <p className="text-[9px] text-slate-500">Cuota {inst.installmentNumber} · ${Number(inst.amount || 0).toLocaleString('es-AR')} · Vence {inst.dueDate}</p>
                  ...
                  {installmentReminderGroups[day].length === 0 && (
                    <p className="text-[10px] text-slate-400 italic">Sin cuotas para este aviso.</p>
                  )}
                </div>
              </button>
            ))}
        ...
        <Card className="h-fit">
          ...
          <div className="space-y-4">
            {reminderDays.map((day) => (
              <div key={`pol-${day}`} className="rounded-2xl border border-slate-100 dark:border-slate-700/50 p-3">
                <div className="flex items-center justify-between gap-2 mb-2">
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{reminderLabel(day)} · {policyReminderGroups[day].length}</p>
                  <button
                    onClick={() => sendPolicyGroupNotifications(day)}
                    className="px-2.5 py-1.5 rounded-lg bg-blue-50 text-blue-600 text-[8px] font-black uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-colors"
                  >
                ...
                <div className="space-y-2">
                  {policyReminderGroups[day].slice(0, 4).map(({ policy, client }) => (
                    <div key={policy.id} className="flex items-center justify-between gap-2 p-2 bg-slate-50 dark:bg-slate-900/50 rounded-xl">
                      <div className="truncate">
                        <p className="text-[10px] font-black text-slate-800 dark:text-white truncate">{client.name} · {policy.matricula}</p>
                        <p className="text-[9px] text-slate-500">{policy.company} · Vence {policy.vigenciaHasta}</p>
                        <p className="text-[8px] font-bold text-slate-400 uppercase tracking-wider">
                      ...
                      <button
                        onClick={() => sendPolicyReminderWhatsApp(client, policy, day)}
                        disabled={getNoticeMeta(policy, 'policy').notifiedToday}
                        className="shrink-0 px-2.5 py-1.5 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-600 hover:text-white disabled:opacity-40 transition-all text-[8px] font-black uppercase tracking-widest"
                  ...
                  {policyReminderGroups[day].length === 0 && (
                    <p className="text-[10px] text-slate-400 italic">Sin pólizas para este aviso.</p>
                  )}
                </div>
              </div>
            ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
         <Card className="h-fit">
            <div className="flex items-center justify-between mb-6">
               ...
               <button onClick={() => navigateToCobros('vencidos', { viewMode: 'list' })} className="text-[10px] font-bold text-blue-600 uppercase">Ver Todos</button>
            </div>
            <div className="space-y-3">
         ...
         <div className="flex flex-col md:flex-row gap-3 items-center justify-between w-full">
            <div className="flex bg-slate-100 dark:bg-slate-900 rounded-xl p-1 w-full md:w-auto overflow-x-auto scrollbar-hide">
               {['todos', 'pendientes', 'pagados', 'vencidos', 'sin_informar'].map(f => (
                 <button key={f} onClick={() => { setCobrosFilter(f); setCobrosReminderScope(null); }} className={`px-4 py-2 rounded-lg text-[9px] sm:text-[10px] font-black uppercase whitespace-nowrap transition-all ${cobrosFilter === f ? 'bg-white dark:bg-slate-800 shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}`}>{f.replace('_', ' ')}</button>
               ))}
            </div>
