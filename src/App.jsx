...
export default function App() {
  ...
  const sendInstallmentReminderWhatsApp = async (client, inst, daysBefore) => {
    if (!client?.phone) {
      showNotify('El cliente no tiene celular cargado', 'error');
      return false;
    }
    if (!canNotifyItemToday(inst, 'installment')) {
  ...
  const sendPendingPaymentWhatsApp = async (pendingEntry) => {
    const { client, overdueInstallments, overduePolicies, maxDaysOverdue } = pendingEntry || {};
    if (!client?.phone) {
      showNotify('El cliente no tiene celular cargado', 'error');
      return false;
    }
    if (client.lastPendingNoticeDay === todayStr) {
        ...
        <Card className="h-fit">
          ...
          <div className="space-y-4">
            {reminderDays.map((day) => (
              <div key={`inst-${day}`} className="rounded-2xl border border-slate-100 dark:border-slate-700/50 p-3">
                <div className="flex items-center justify-between gap-2 mb-2">
                  ...
                  <button
                    onClick={() => sendInstallmentGroupNotifications(day)}
                ...
                <div className="space-y-2">
                  {installmentReminderGroups[day].slice(0, 4).map(({ inst, client }) => (
                    <div key={inst.id} className="flex items-center justify-between gap-2 p-2 bg-slate-50 dark:bg-slate-900/50 rounded-xl">
                      ...
                      <button
                        onClick={() => sendInstallmentReminderWhatsApp(client, inst, day)}
                        disabled={getNoticeMeta(inst, 'installment').notifiedToday}
      ...
      <Card className="h-fit">
        ...
        <div className="space-y-3">
          {pendingPaymentClients.slice(0, 8).map((entry) => (
            <div key={entry.clientId} className="flex items-center justify-between gap-3 p-3 rounded-2xl border border-rose-100 dark:border-rose-900/40 bg-rose-50/40 dark:bg-rose-900/10">
              ...
              <button
                onClick={() => sendPendingPaymentWhatsApp(entry)}
                disabled={entry.client?.lastPendingNoticeDay === todayStr}
                className="shrink-0 px-2.5 py-1.5 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-600 hover:text-white disabled:opacity-40 transition-all text-[8px] font-black uppercase tracking-widest"
              >
                Notificar
              </button>
