...
const ROLE_PERMISSIONS = {
  admin: { ...BASE_PERMISSIONS, reports: true, manageUsers: true, manageClaims: true },
  agent: { ...BASE_PERMISSIONS, reports: false, settings: false, manageUsers: false, manageClaims: false }
};

const VEHICLE_BRAND_MODELS = {
  Toyota: ['Corolla', 'Hilux', 'Etios', 'Yaris', 'SW4', 'RAV4', 'Otro'],
  Ford: ['Fiesta', 'Focus', 'Ranger', 'EcoSport', 'Ka', 'Maverick', 'Otro'],
  Chevrolet: ['Onix', 'Cruze', 'S10', 'Tracker', 'Spin', 'Prisma', 'Otro'],
  Volkswagen: ['Gol', 'Polo', 'Vento', 'Amarok', 'T-Cross', 'Nivus', 'Otro'],
  Fiat: ['Cronos', 'Toro', 'Argo', 'Strada', 'Mobi', 'Pulse', 'Otro'],
  Renault: ['Kwid', 'Sandero', 'Logan', 'Duster', 'Oroch', 'Stepway', 'Otro'],
  Peugeot: ['208', '2008', '3008', 'Partner', '408', 'Otro'],
  Citroen: ['C3', 'C4 Cactus', 'Berlingo', 'Otro'],
  Honda: ['Civic', 'City', 'HR-V', 'CR-V', 'Fit', 'Otro'],
  Nissan: ['Versa', 'Sentra', 'Frontier', 'Kicks', 'March', 'Otro'],
  Mercedes: ['Sprinter', 'Clase A', 'Clase C', 'Clase GLA', 'Otro'],
  BMW: ['Serie 1', 'Serie 3', 'X1', 'X3', 'Otro'],
  Audi: ['A1', 'A3', 'A4', 'Q3', 'Q5', 'Otro'],
  Jeep: ['Renegade', 'Compass', 'Cherokee', 'Wrangler', 'Otro'],
  Kia: ['Rio', 'Cerato', 'Sportage', 'Seltos', 'Otro'],
  Hyundai: ['HB20', 'Creta', 'Tucson', 'Santa Fe', 'Otro'],
  Yamaha: ['YBR 125', 'FZ', 'XTZ', 'R3', 'Otro'],
  HondaMoto: ['Wave', 'CG Titan', 'Biz', 'Tornado', 'CBR', 'Otro'],
  Zanella: ['ZB', 'RX', 'Patagonian', 'Otro'],
  Motomel: ['B110', 'Skua', 'S2', 'Otra'],
  Iveco: ['Daily', 'Tector', 'Stralis', 'Otro'],
  Scania: ['P', 'G', 'R', 'Otro'],
  Volvo: ['FH', 'FM', 'Otro'],
  Otro: ['Otro']
};
...
export default function App() {
  ...
  const [clientsScope, setClientsScope] = useState('all');
  const [viewMode, setViewMode] = useState('grid'); 
  const [cobrosFilter, setCobrosFilter] = useState('todos');
  const [cobrosReminderScope, setCobrosReminderScope] = useState(null);
  ...
  const [vehicleBrandForm, setVehicleBrandForm] = useState('Toyota');
  const [claimForm, setClaimForm] = useState({ date: '', policyId: '', amount: '', notes: '' });
  ...
  const [agencySettings, setAgencySettings] = useState({
    name: 'Se-Flow', logo: '', phone: '', address: '', paymentInstructions: '', installmentNotifyMode: 'manual', policyNotifyMode: 'manual', notifyHour: '09:00', reminderDays: [3, 1, 0] 
  });
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (u) => {
      ...
      if (u) {
        ...
        onSnapshot(collection(db, 'artifacts', appId, 'public', 'data', 'claims'), (snap) => {
  ...
  const policiesByClientId = useMemo(
    () => policies.reduce((acc, policy) => {
      if (!acc[policy.clientId]) acc[policy.clientId] = [];
      acc[policy.clientId].push(policy);
      return acc;
    }, {}),
    [policies]
  );

  const installmentsByPolicyId = useMemo(
    () => processedInstallments.reduce((acc, installment) => {
      if (!acc[installment.policyId]) acc[installment.policyId] = [];
      acc[installment.policyId].push(installment);
      return acc;
    }, {}),
  ...
  const filteredInstallments = useMemo(() => {
    ...
    if (cobrosReminderScope !== null) {
      result = result.filter((inst) => daysUntilDate(inst.dueDate) === cobrosReminderScope);
    }
    if (searchTerm) {
      const lowerTerm = searchTerm.toLowerCase();
      result = result.filter(i => {
         const client = clientsById[i.clientId];
  ...
  }, [processedInstallments, cobrosFilter, cobrosReminderScope, searchTerm, clientsById]);
  ...
  const getCoverageStatus = (policy) => {
    ...
    const polInsts = (installmentsByPolicyId[policy.id] || []).filter(i => i.computedStatus === 'sin_informar' || i.status === 'vencido');
  ...
  const getNoticeMeta = (item, type = 'installment') => {
    if (!item) return { notifiedToday: false, count: 0 };
    const dayField = type === 'policy' ? item.lastPolicyNoticeDay : item.lastInstallmentNoticeDay;
    const countField = type === 'policy' ? item.policyNoticeCount : item.installmentNoticeCount;
    return {
      notifiedToday: dayField === todayStr,
  ...
  const pendingPaymentClients = useMemo(() => {
    const byClient = {};
    const ensureClient = (clientId) => {
    ...
    processedInstallments.forEach((inst) => {
      const isUnpaid = inst.status !== 'pagado' && (inst.computedStatus === 'sin_informar' || inst.status === 'vencido');
      if (!isUnpaid) return;
      const diff = getDaysFromToday(inst.dueDate);
      const daysOverdue = diff !== null && diff < 0 ? Math.abs(diff) : 0;
      const holder = ensureClient(inst.clientId);
      holder.overdueInstallments.push({ ...inst, daysOverdue });
      holder.maxDaysOverdue = Math.max(holder.maxDaysOverdue, daysOverdue);
    });

    policies.forEach((policy) => {
      if (policy.status === 'baja') return;
      const diff = getDaysFromToday(policy.vigenciaHasta);
      if (diff === null || diff >= 0) return;
      const daysOverdue = Math.abs(diff);
      const holder = ensureClient(policy.clientId);
      holder.overduePolicies.push({ ...policy, daysOverdue });
      holder.maxDaysOverdue = Math.max(holder.maxDaysOverdue, daysOverdue);
    });

    return Object.values(byClient)
      .map((entry) => ({ ...entry, client: clientsById[entry.clientId] }))
      .filter(entry => entry.client && entry.client.status !== 'baja')
      .sort((a, b) => b.maxDaysOverdue - a.maxDaysOverdue);
  }, [clientsById, policies, processedInstallments]);

  const pendingClientIds = useMemo(
    () => new Set(pendingPaymentClients.map(entry => entry.clientId)),
    [pendingPaymentClients]
  );
  const pendingPaymentByClientId = useMemo(
    () => pendingPaymentClients.reduce((acc, entry) => {
  ...
  const urgentInstallments = useMemo(
    () => processedInstallments
      .filter((inst) => inst.status !== 'pagado' && (inst.computedStatus === 'sin_informar' || inst.status === 'vencido'))
      .sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate)),
    [processedInstallments]
  );

  const overdueInstallments = useMemo(
    () => processedInstallments.filter((inst) => inst.status === 'vencido' || inst.computedStatus === 'sin_informar'),
    [processedInstallments]
  );

  const filteredClients = useMemo(() => {
    const base = clients.filter(c => c.status !== 'baja' && (clientsScope === 'all' || pendingClientIds.has(c.id)));
    if (!searchTerm) return base;
    const lowerTerm = searchTerm.toLowerCase();
    return base.filter(c => {
       const matchName = c.name.toLowerCase().includes(lowerTerm);
       const matchDni = c.dni?.includes(lowerTerm);
       const clientPols = policiesByClientId[c.id] || [];
       const matchPol = clientPols.some(p => p.matricula.toLowerCase().includes(lowerTerm));
       return matchName || matchDni || matchPol;
    });
  }, [clients, searchTerm, policiesByClientId, clientsScope, pendingClientIds]);
  ...
  const dashMetrics = useMemo(() => {
    const polNoBaja = policies.filter(p => p.status !== 'baja');
    const vigentes = polNoBaja.filter(p => p.vigenciaHasta >= todayStr).length;
    const vencidas = overdueInstallments.length;
    const renovarHoy = polNoBaja.filter(p => p.status === 'activa' && p.vigenciaHasta === todayStr).length;
    ...
    const proxRenov = polNoBaja.filter(p => { const end = new Date(p.vigenciaHasta); return end >= nextMonth && end <= endOfNextMonth; }).length;

    return { vigentes, vencidas, renovarHoy, sinInformar, proxRenov, clientesConPendientes: pendingPaymentClients.length };
  }, [policies, processedInstallments, todayStr, pendingPaymentClients.length, overdueInstallments.length]);
  ...
  const installmentReminderGroups = useMemo(() => {
    const groups = { 5: [], 2: [], 0: [] };
    if (agencySettings.installmentNotifyMode === 'off') return groups;
    processedInstallments.forEach((inst) => {
      if (inst.status !== 'pendiente') return;
      const diff = daysUntilDate(inst.dueDate);
      if (!reminderDays.includes(diff)) return;
      const client = clientsById[inst.clientId];
      if (!client?.phone) return;
      groups[diff].push({ inst, client, policy: policiesById[inst.policyId] || null });
    });
    return groups;
  }, [processedInstallments, clientsById, policiesById, agencySettings.installmentNotifyMode]);

  const policyReminderGroups = useMemo(() => {
    const groups = { 5: [], 2: [], 0: [] };
    if (agencySettings.policyNotifyMode === 'off') return groups;
    policies.forEach((policy) => {
      if (policy.status !== 'activa') return;
      const diff = daysUntilDate(policy.vigenciaHasta);
      if (!reminderDays.includes(diff)) return;
      const client = clientsById[policy.clientId];
      if (!client?.phone) return;
      groups[diff].push({ policy, client });
    });
    return groups;
  }, [policies, clientsById, agencySettings.policyNotifyMode]);

  const canNotifyItemToday = (item, type = 'installment') => {
    if (!item) return false;
    const field = type === 'policy' ? item.lastPolicyNoticeDay : item.lastInstallmentNoticeDay;
    return field !== todayStr;
  };
  ...
  const canManageClaims = appUser?.permissions?.manageClaims || appUser?.role === 'admin';

  const selectedClientClaims = useMemo(() => {
    if (!selectedClient?.id) return [];
    return claims
      .filter(cl => cl.clientId === selectedClient.id)
      .sort((a, b) => {
        const da = toDateSafe(a.date || a.createdAt)?.getTime() || 0;
        const dbv = toDateSafe(b.date || b.createdAt)?.getTime() || 0;
        return dbv - da;
      });
  }, [claims, selectedClient]);

  const selectedClientClaimsSummary = useMemo(() => {
    const total = selectedClientClaims.length;
    const totalAmount = selectedClientClaims.reduce((acc, cl) => acc + (parseFloat(cl.amount) || 0), 0);
    return { total, totalAmount };
  }, [selectedClientClaims]);
  const selectedClientPolicies = useMemo(
    () => (selectedClient?.id ? (policiesByClientId[selectedClient.id] || []) : []),
    [selectedClient, policiesByClientId]
  );
  const policiesRenewingToday = useMemo(
    () => policies.filter(p => p.vigenciaHasta === todayStr && p.status === 'activa'),
    [policies, todayStr]
  );
  const vehicleModelOptions = useMemo(() => VEHICLE_BRAND_MODELS[vehicleBrandForm] || ['Otro'], [vehicleBrandForm]);
  ...
  useEffect(() => {
    if (!selectedClient?.id) {
      setClaimForm({ date: '', policyId: '', amount: '', notes: '' });
      return;
    }
    const pol = policies.find(p => p.clientId === selectedClient.id);
    setClaimForm(prev => ({ ...prev, policyId: pol?.id || '' }));
  ...
  const handleInstallApp = async () => {
    ...
    if (!promptEvent) {
      ...
      setInstallHintMessage(
        isDesktopChrome
          ? 'Chrome aún no habilitó el prompt automático. Si estás viendo Se-Flow dentro del panel integrado, allí no aparece la opción de instalar: abre esta misma URL en Chrome o Edge y usa el icono de instalar o el menú ⋮.'
          : 'Este navegador no habilitó el prompt automático. Si estás dentro del panel integrado, abre esta misma URL en Chrome, Edge o Safari para instalar.'
      );
  ...
  const navigateToCobros = (filter, options = {}) => {
    setActiveTab('vencimientos');
    setCobrosFilter(filter);
    setCobrosReminderScope(options.reminderDay ?? null);
    setSearchTerm(options.searchTerm ?? '');
    if (options.viewMode) setViewMode(options.viewMode);
    setSidebarOpen(false);
  };
  const navigateToPolicies = (term) => { setActiveTab('clients'); setClientsScope('all'); setSearchTerm(term); setSidebarOpen(false); };
  const navigateToPendingClients = () => {
    setSelectedClient(null);
    setIsDetailOpen(false);
    setIsEditClientOpen(false);
    setIsRenewOpen(false);
    setActiveTab('clients');
    setClientsScope('pending');
    setSearchTerm('');
    setSidebarOpen(false);
  };
  const navigateToInstallmentReminderList = (day) => {
    navigateToCobros('pendiente', { reminderDay: day, viewMode: 'list' });
  };
  const navigateToPolicyReminderList = () => {
    navigateToPolicies('');
  };
  const clearCobrosReminderScope = () => setCobrosReminderScope(null);
  ...
  const executeSavePolicy = async (formDataObj, overrideClientId = null) => {
    const matricula = normalizePlate(formDataObj.matricula);
    const montoCuota = parseFloat(formDataObj.montoCuota) || 0;
    const normalizedDoc = normalizeDocumentId(formDataObj.dni);
    const vehicleBrand = formDataObj.vehicleBrand || 'Otro';
    const vehicleModel = formDataObj.vehicleModel || '';
    const vehicleYear = String(formDataObj.vehicleYear || '').trim();
    try {
      let clientId = overrideClientId || selectedClientForPolicy?.id;
      if (!clientId) {
        const matchedClient = clients.find((c) => {
          if (c.status === 'baja') return false;
          const existingDoc = c.docIdNormalized || normalizeDocumentId(c.dni);
          if (normalizedDoc && existingDoc) return existingDoc === normalizedDoc;
          return normalizeName(c.name) === normalizeName(formDataObj.name);
        });
        if (matchedClient) {
      ...
      if (!clientId) {
        const clientDoc = await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'clients'), {
          ...
          dni: formDataObj.dni,
          docIdNormalized: normalizedDoc,
          phone: formDataObj.phone,
          landline: formDataObj.landline,
          email: formDataObj.email,
          street: formDataObj.street,
          door: formDataObj.door,
          locality: formDataObj.locality,
      ...
      const alreadyHasPolicy = policies.some((p) => p.clientId === clientId && normalizePlate(p.matricula) === matricula);
      if (alreadyHasPolicy) {
        showNotify(`La matrícula ${matricula} ya está asociada a este cliente`, 'info');
      ...
      const policyDoc = await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'policies'), {
        ...
        vehicleType: formDataObj.vehicleType,
        vehicleBrand,
        vehicleModel,
        vehicleYear,
        vigenciaDesde,
      ...
      setModalOpen(false); setSelectedClientForPolicy(null); setVigenciaDesde(''); setVigenciaHasta(''); setPolicyPhotos([]); setPolicyLink(''); setPolicyPdf(''); setQuickSharePhone(''); setVehicleBrandForm('Toyota');
  ...
  const handleCreateOrUpdateClientAndPolicy = async (e) => {
    ...
    if (!selectedClientForPolicy && formDataObj.dni) {
      const normalizedDoc = normalizeDocumentId(formDataObj.dni);
      const existingClient = clients.find(c => c.status !== 'baja' && (c.docIdNormalized || normalizeDocumentId(c.dni)) === normalizedDoc);
  ...
  const sendWhatsApp = async (client, inst) => {
    ...
    const linkedPolicy = policiesById[inst.policyId];
    const paymentLink = linkedPolicy?.linkPago || 'Coordina el pago con la agencia.';
    const paymentInstructions = agencySettings.paymentInstructions?.trim();
    const paymentBlock = paymentInstructions ? `\nMedio de pago sugerido: ${paymentInstructions}` : '';
    const msg = `Hola ${client.name}, esperamos que estés muy bien. Te recordamos que tu cuota mensual ${inst.installmentNumber} de la póliza (${inst.matricula}) está ${inst.computedStatus === 'sin_informar' ? 'vencida/sin informar' : 'próxima a vencer'}. Importe: $${(parseFloat(inst.amount) || 0).toLocaleString('es-AR')}.${paymentBlock}\nLink de pago: ${paymentLink}\nPor favor, una vez abonado, envíanos el comprobante por este medio. Muchas gracias.`;
  ...
  const sendInstallmentReminderWhatsApp = async (client, inst, daysBefore, options = {}) => {
    const force = Boolean(options.force);
    if (!client?.phone) {
      showNotify('El cliente no tiene celular cargado', 'error');
      return false;
    }
    if (!force && !canNotifyItemToday(inst, 'installment')) {
      showNotify(`La cuota ${inst.installmentNumber} de ${inst.matricula} ya fue notificada hoy`, 'info');
      return false;
    }
    const clean = client.phone.replace(/\D/g, '');
    const linkedPolicy = policiesById[inst.policyId];
    const paymentLink = linkedPolicy?.linkPago || 'Coordina el pago con la agencia.';
    const paymentInstructions = agencySettings.paymentInstructions?.trim();
    const dueText = daysBefore === 0 ? 'vence hoy' : `vence en ${daysBefore} días`;
    const msg = `Hola ${client.name}, esperamos que estés muy bien.\nTe recordamos que tu cuota mensual del seguro (${inst.matricula}) ${dueText}.\nImporte: $${(parseFloat(inst.amount) || 0).toLocaleString('es-AR')}.\n${paymentInstructions ? `Para ayudarte con el pago te dejamos: ${paymentInstructions}.\n` : ''}Link de pago: ${paymentLink}\nPor favor envíanos el comprobante cuando realices el pago.\n\nSaludos cordiales,\n${agencySettings.name || 'Se-Flow'}`;
    window.open(`https://wa.me/${clean}?text=${encodeURIComponent(msg)}`, '_blank');
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'installments', inst.id), {
      lastInstallmentNoticeAt: new Date().toISOString(),
      lastInstallmentNoticeDay: todayStr,
      installmentNoticeCount: Number(inst.installmentNoticeCount || 0) + 1
    });
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'clients', client.id), { lastNotified: new Date().toISOString() });
    await logAction('Recordatorio Cuota WhatsApp', `${client.name} · ${inst.matricula} · ${dueText}`);
    return true;
  };

  const sendPolicyReminderWhatsApp = async (client, policy, daysBefore) => {
    if (!client?.phone) {
      showNotify('El cliente no tiene celular cargado', 'error');
      return false;
    }
    if (!canNotifyItemToday(policy, 'policy')) {
      showNotify(`La póliza ${policy.matricula} ya fue notificada hoy`, 'info');
      return false;
    }
    const clean = client.phone.replace(/\D/g, '');
    const dueText = daysBefore === 0 ? 'vence hoy' : `vence en ${daysBefore} días`;
    const msg = `Hola ${client.name}, esperamos que estés muy bien.\nTe recordamos que la póliza de ${policy.matricula} (${policy.company}) ${dueText}.\nFecha de vencimiento: ${policy.vigenciaHasta}.\nPara continuar con cobertura activa, te recomendamos renovar a la brevedad.\n\nSaludos cordiales,\n${agencySettings.name || 'Se-Flow'}`;
    window.open(`https://wa.me/${clean}?text=${encodeURIComponent(msg)}`, '_blank');
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'policies', policy.id), {
      lastPolicyNoticeAt: new Date().toISOString(),
      lastPolicyNoticeDay: todayStr,
      policyNoticeCount: Number(policy.policyNoticeCount || 0) + 1
    });
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'clients', client.id), { lastNotified: new Date().toISOString() });
    await logAction('Recordatorio Póliza WhatsApp', `${client.name} · ${policy.matricula} · ${dueText}`);
    return true;
  };

  const sendPendingPaymentWhatsApp = async (pendingEntry, options = {}) => {
    const force = Boolean(options.force);
    const { client, overdueInstallments, overduePolicies, maxDaysOverdue } = pendingEntry || {};
    if (!client?.phone) {
      showNotify('El cliente no tiene celular cargado', 'error');
      return false;
    }
    if (!force && client.lastPendingNoticeDay === todayStr) {
      showNotify(`${client.name} ya fue notificado hoy por pagos pendientes`, 'info');
      return false;
    }
    const clean = client.phone.replace(/\D/g, '');
    const unpaidInstallments = overdueInstallments?.length || 0;
    const expiredPolicies = overduePolicies?.length || 0;
    const paymentInstructions = agencySettings.paymentInstructions?.trim();
    const msg = `Hola ${client.name}, esperamos que estés muy bien.\nTe escribimos para recordarte amablemente que registramos pagos pendientes de tu seguro desde hace ${maxDaysOverdue} día(s).\nCuotas pendientes: ${unpaidInstallments}.\nPólizas vencidas: ${expiredPolicies}.\n${paymentInstructions ? `Para ayudarte con el pago te dejamos este medio: ${paymentInstructions}.\n` : ''}Por favor, cuando puedas, regularizalo y envíanos el comprobante por este medio.\n\nSaludos cordiales,\n${agencySettings.name || 'Se-Flow'}`;
    window.open(`https://wa.me/${clean}?text=${encodeURIComponent(msg)}`, '_blank');
    await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'clients', client.id), {
      lastNotified: new Date().toISOString(),
      lastPendingNoticeDay: todayStr,
      pendingNoticeCount: Number(client.pendingNoticeCount || 0) + 1
    });
    await logAction('Recordatorio Mora WhatsApp', `${client.name} · ${maxDaysOverdue} día(s)`);
    return true;
  };

  const handleInstallmentNotification = async (inst, options = {}) => {
    const client = clientsById[inst.clientId];
    if (!client?.phone) {
      showNotify('El cliente no tiene celular cargado', 'error');
      return false;
    }
    const dueDiff = daysUntilDate(inst.dueDate);
    if (inst.status === 'pendiente' && reminderDays.includes(dueDiff)) {
      return await sendInstallmentReminderWhatsApp(client, inst, dueDiff, options);
    }
    await sendWhatsApp(client, inst);
    return true;
  };

  const sendInstallmentGroupNotifications = async (day) => {
    const group = installmentReminderGroups[day] || [];
    const pendingToSend = group.filter(({ inst }) => canNotifyItemToday(inst, 'installment'));
    if (!pendingToSend.length) {
      showNotify('No hay cuotas nuevas para notificar hoy en este grupo', 'info');
      return;
    }
    let sent = 0;
    for (const item of pendingToSend) {
      const ok = await sendInstallmentReminderWhatsApp(item.client, item.inst, day);
      if (ok) sent += 1;
    }
    showNotify(`Notificaciones enviadas: ${sent}/${pendingToSend.length}`);
  };

  const sendPolicyGroupNotifications = async (day) => {
    const group = policyReminderGroups[day] || [];
    const pendingToSend = group.filter(({ policy }) => canNotifyItemToday(policy, 'policy'));
    if (!pendingToSend.length) {
      showNotify('No hay pólizas nuevas para notificar hoy en este grupo', 'info');
      return;
    }
    let sent = 0;
    for (const item of pendingToSend) {
      const ok = await sendPolicyReminderWhatsApp(item.client, item.policy, day);
      if (ok) sent += 1;
    }
    showNotify(`Notificaciones enviadas: ${sent}/${pendingToSend.length}`);
  };
  ...
  const exportToExcel = () => {
    ...
    const rows = filteredClients.map(c => {
      ...
      const clientPolicies = policiesByClientId[c.id] || [];
  ...
  const normalizeDocumentId = (rawDoc = '') => String(rawDoc || '').replace(/\D/g, '');
  const normalizeName = (rawName = '') => String(rawName || '').trim().toLowerCase();
  const normalizePlate = (rawPlate = '') => String(rawPlate || '').trim().toUpperCase();
  ...
  const handleQuickSharePolicyInfo = async () => {
    ...
    const message = [
      ...
      `Marca / Modelo / Año: ${getValue('vehicleBrand') || '-'} / ${getValue('vehicleModel') || '-'} / ${getValue('vehicleYear') || '-'}`,
  ...
  const executeCsvImport = async () => {
    ...
    clients.forEach((c) => {
      if (c.status === 'baja') return;
      const docKey = c.docIdNormalized || normalizeDocumentId(c.dni);
      if (docKey) clientByDoc.set(docKey, c.id);
      const nameKey = normalizeName(c.name);
      if (nameKey) clientByName.set(nameKey, c.id);
    });
    const policyKeySet = new Set(
      policies.map((p) => `${p.clientId}::${normalizePlate(p.matricula)}`)
    );
    ...
    for (let i = 0; i < total; i++) {
      ...
      try {
        ...
        const name = getVal('name'); const matricula = normalizePlate(getVal('matricula'));
        if (!name || !matricula) continue; 

        let clientId = null; const dni = getVal('dni');
        const normalizedDoc = normalizeDocumentId(dni);
        const normalizedRowName = normalizeName(name);
        if (normalizedDoc && clientByDoc.has(normalizedDoc)) {
          clientId = clientByDoc.get(normalizedDoc);
        } else if (normalizedRowName && clientByName.has(normalizedRowName)) {
          clientId = clientByName.get(normalizedRowName);
        }
        ...
        if (!clientId) {
           ...
           clientId = cRef.id;
           if (normalizedDoc) clientByDoc.set(normalizedDoc, clientId);
           if (normalizedRowName) clientByName.set(normalizedRowName, clientId);
        }

        const policyKey = `${clientId}::${matricula}`;
        if (policyKeySet.has(policyKey)) {
          skippedCount++;
          continue;
        }
        ...
        const pRef = await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'policies'), {
           clientId,
           matricula,
           company: getVal('company') || 'Otra',
           vehicleType: getVal('vehicleType') || 'Auto',
           vehicleBrand: getVal('vehicleBrand') || 'Otro',
           vehicleModel: getVal('vehicleModel') || '',
           vehicleYear: getVal('vehicleYear') || '',
    ...
    logAction('Importación Masiva', `${successCount} vehículos importados, ${skippedCount} omitidos por matrícula duplicada`);
    showNotify(`Importación completada: ${successCount} vehículos procesados (${skippedCount} omitidos por duplicados).`);
  };
  ...
  const handleEditClientSubmit = async (e) => {
    ...
    try {
      await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'clients', selectedClient.id), {
        name: fd.get('name'),
        dni: fd.get('dni'),
        docIdNormalized: normalizeDocumentId(fd.get('dni')),
        phone: fd.get('phone'),
        landline: fd.get('landline'),
        email: fd.get('email'),
  ...
  const updateClientRiskCategory = async (clientId, riskCategory) => {
    if (!canManageClaims) {
      showNotify('No tienes permiso para clasificar clientes', 'error');
      return;
    }
    try {
      await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'clients', clientId), { riskCategory });
      await logAction('Clasificación Riesgo', `Cliente ${clientId} => ${riskCategory}`);
      showNotify('Clasificación de riesgo actualizada');
    } catch (error) {
      console.error(error);
      showNotify('No se pudo actualizar la clasificación', 'error');
    }
  };

  const addClientClaim = async () => {
    if (!canManageClaims) {
      showNotify('No tienes permiso para cargar siniestros', 'error');
      return;
    }
    if (!selectedClient?.id) {
      showNotify('Selecciona un cliente primero', 'error');
      return;
    }
    const date = claimForm.date || new Date().toISOString().slice(0, 10);
    const amount = claimForm.amount ? parseFloat(claimForm.amount) : null;
    const policyId = claimForm.policyId || '';
    try {
      await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'claims'), {
        clientId: selectedClient.id,
        policyId,
        policyMatricula: policiesById[policyId]?.matricula || '',
        date,
        amount: Number.isFinite(amount) ? amount : null,
        notes: (claimForm.notes || '').trim(),
        createdAt: serverTimestamp(),
        createdBy: appUser?.name || 'Sistema'
      });
      const clientClaims = claims.filter(c => c.clientId === selectedClient.id);
      const nextCount = clientClaims.length + 1;
      const currentTotal = clientClaims.reduce((acc, c) => acc + (parseFloat(c.amount) || 0), 0);
      const nextTotal = currentTotal + (Number.isFinite(amount) ? amount : 0);
      await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'clients', selectedClient.id), {
        claimsCount: nextCount,
        claimsAmountTotal: nextTotal
      });
      await logAction('Alta Siniestro', `Cliente ${selectedClient.name} · ${policiesById[policyId]?.matricula || 'Sin matrícula'} · $${Number.isFinite(amount) ? amount : 0}`);
      setClaimForm({ date: '', policyId: policyId || '', amount: '', notes: '' });
      showNotify('Siniestro registrado');
    } catch (error) {
      console.error(error);
      showNotify('No se pudo registrar el siniestro', 'error');
    }
  };
  ...
  const backgroundUrlRegex = /url\((['"]?)(.*?)\1\)/i;

  const getBackgroundUrl = (backgroundImage = '') => {
    const match = String(backgroundImage || '').match(backgroundUrlRegex);
    return match?.[2] || '';
  };

  const removeUrlFromBackgroundImage = (backgroundImage = '') => {
    const cleaned = String(backgroundImage || '')
      .replace(/\s*,?\s*url\((['"]?)(.*?)\1\)\s*,?/i, '')
      .replace(/^,|,$/g, '')
      .trim();
    return cleaned || 'none';
  };
    ...
    return await new Promise((resolve, reject) => {
      ...
      image.onload = () => {
        try {
          ...
          if (!ctx) {
            reject(new Error('No se pudo crear contexto para convertir imagen'));
            return;
          }
  ...
  const prepareCloneAssetsForExport = async (clone, { stripExternalImages = false } = {}) => {
    const imageNodes = Array.from(clone.querySelectorAll('img'));
    ...
    const nodesWithBackground = [clone, ...Array.from(clone.querySelectorAll('*'))];
    for (const node of nodesWithBackground) {
      const backgroundImage = node.style?.backgroundImage || '';
      const bgUrl = getBackgroundUrl(backgroundImage);
      if (!isExternalImageSrc(bgUrl)) continue;
      if (stripExternalImages) {
        node.style.backgroundImage = removeUrlFromBackgroundImage(backgroundImage);
        continue;
      }
      try {
        const dataUrl = await imageUrlToDataUrl(bgUrl);
        node.style.backgroundImage = backgroundImage.replace(backgroundUrlRegex, `url("${dataUrl}")`);
      } catch (_) {
        // Dejamos el valor original para el intento principal.
      }
    }
  };
  ...
  const renderFlyerCanvas4K = async ({ stripExternalImages = false } = {}) => {
    ...
    await prepareCloneAssetsForExport(clone, { stripExternalImages });
  ...
  const handleDownloadFlyer4K = async () => {
    try {
      ...
      showNotify('No se pudo generar el flyer 4K. Revisa logo, banner o fondo externo.', 'error');
    }
  };
  ...
  const renderDashboard = () => (
    <div className="space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-700 no-print">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4">
        ...
        <Card onClick={() => navigateToCobros('vencido', { viewMode: 'list' })} className="bg-rose-50 dark:bg-rose-900/10 border-rose-100 dark:border-rose-900/30 group flex flex-col justify-between">
          <div className="flex items-center justify-between mb-2"><p className="text-[9px] sm:text-[10px] font-black text-rose-500 uppercase tracking-widest leading-tight">Cobros<br/>Vencidos</p><AlertCircle size={16} className="text-rose-500/40" /></div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 sm:gap-8">
        <Card className="h-fit">
          <div className="flex items-center justify-between mb-4">
            <h4 className="text-sm sm:text-md font-black text-slate-800 dark:text-white uppercase tracking-widest flex items-center gap-2">
              <Clock size={16} className="text-blue-500" /> Avisos Cuotas Mensuales
            </h4>
          </div>
          <div className="space-y-2.5 sm:space-y-3">
            {reminderDays.map((day) => (
              <button
                key={`inst-${day}`}
                type="button"
                onClick={() => navigateToInstallmentReminderList(day)}
                className="w-full text-left rounded-xl sm:rounded-2xl border border-slate-100 dark:border-slate-700/50 px-3 py-2.5 sm:p-3 hover:border-blue-200 hover:bg-blue-50/30 dark:hover:bg-slate-900/40 transition-colors"
              >
                <div className="flex items-center justify-between gap-2 mb-1.5">
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{reminderLabel(day)}</p>
                  <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest">{installmentReminderGroups[day].length}</span>
                </div>
                <div className="space-y-1.5">
                  {installmentReminderGroups[day].slice(0, 4).map(({ inst, client }) => (
                    <div key={inst.id} className="flex items-center justify-between gap-2 p-2 bg-slate-50 dark:bg-slate-900/50 rounded-lg sm:rounded-xl">
                      <div className="truncate">
                        <p className="text-[10px] font-black text-slate-800 dark:text-white truncate">{client.name} · {inst.matricula}</p>
                        <p className="text-[8px] sm:text-[9px] text-slate-500">Cuota {inst.installmentNumber} · ${Number(inst.amount || 0).toLocaleString('es-AR')} · Vence {inst.dueDate}</p>
                        <p className="text-[8px] font-bold text-slate-400 uppercase tracking-wider">
                          Avisos enviados: {getNoticeMeta(inst, 'installment').count}
                          {getNoticeMeta(inst, 'installment').notifiedToday ? ' · Notificado hoy' : ''}
                        </p>
                      </div>
                      <span className="shrink-0 px-2 py-1 bg-emerald-50 text-emerald-600 rounded-lg text-[8px] font-black uppercase tracking-widest">
                        WhatsApp
                      </span>
                    </div>
                  ))}
                  {installmentReminderGroups[day].length === 0 && (
                    <p className="text-[9px] sm:text-[10px] text-slate-400 italic">Sin cuotas para este aviso.</p>
                  )}
                </div>
              </button>
            ))}
        ...
        <Card className="h-fit">
          <div className="flex items-center justify-between mb-4">
            <h4 className="text-sm sm:text-md font-black text-slate-800 dark:text-white uppercase tracking-widest flex items-center gap-2">
              <RefreshCcw size={16} className="text-indigo-500" /> Avisos Renovación Pólizas
            </h4>
          </div>
          <div className="space-y-2.5 sm:space-y-3">
            {reminderDays.map((day) => (
              <button
                key={`pol-${day}`}
                type="button"
                onClick={navigateToPolicyReminderList}
                className="w-full text-left rounded-xl sm:rounded-2xl border border-slate-100 dark:border-slate-700/50 px-3 py-2.5 sm:p-3 hover:border-indigo-200 hover:bg-indigo-50/20 dark:hover:bg-slate-900/40 transition-colors"
              >
                <div className="flex items-center justify-between gap-2 mb-1.5">
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{reminderLabel(day)}</p>
                  <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">{policyReminderGroups[day].length}</span>
                </div>
                <div className="space-y-1.5">
                  {policyReminderGroups[day].slice(0, 4).map(({ policy, client }) => (
                    <div key={policy.id} className="flex items-center justify-between gap-2 p-2 bg-slate-50 dark:bg-slate-900/50 rounded-lg sm:rounded-xl">
                      <div className="truncate">
                        <p className="text-[10px] font-black text-slate-800 dark:text-white truncate">{client.name} · {policy.matricula}</p>
                        <p className="text-[8px] sm:text-[9px] text-slate-500">{policy.company} · Vence {policy.vigenciaHasta}</p>
                        <p className="text-[8px] font-bold text-slate-400 uppercase tracking-wider">
                          Avisos enviados: {getNoticeMeta(policy, 'policy').count}
                          {getNoticeMeta(policy, 'policy').notifiedToday ? ' · Notificado hoy' : ''}
                        </p>
                      </div>
                      <span className="shrink-0 px-2 py-1 bg-emerald-50 text-emerald-600 rounded-lg text-[8px] font-black uppercase tracking-widest">
                        WhatsApp
                  ...
                  {policyReminderGroups[day].length === 0 && (
                    <p className="text-[9px] sm:text-[10px] text-slate-400 italic">Sin pólizas para este aviso.</p>
                  )}
                </div>
              </button>
            ))}
      ...
      <Card className="h-fit">
        <div className="flex items-center justify-between mb-5">
          <h4 className="text-sm sm:text-md font-black text-slate-800 dark:text-white uppercase tracking-widest flex items-center gap-2">
            <AlertCircle size={16} className="text-rose-500" /> Clientes con pago pendiente
          </h4>
          <button onClick={navigateToPendingClients} className="text-[10px] font-bold text-blue-600 uppercase">Ver listado completo</button>
        </div>
        <div className="space-y-3">
          {pendingPaymentClients.slice(0, 8).map((entry) => (
            <button
              key={entry.clientId}
              type="button"
              onClick={navigateToPendingClients}
              className="w-full text-left flex items-center justify-between gap-3 p-3 rounded-2xl border border-rose-100 dark:border-rose-900/40 bg-rose-50/40 dark:bg-rose-900/10 hover:border-rose-200 transition-colors"
            >
              <div className="min-w-0">
                <p className="text-[10px] font-black text-slate-900 dark:text-white uppercase truncate">{entry.client?.name}</p>
                <p className="text-[9px] text-rose-600 font-bold uppercase truncate">
                  Atraso máximo: {entry.maxDaysOverdue} día(s) · Cuotas: {entry.overdueInstallments.length} · Pólizas: {entry.overduePolicies.length}
                </p>
                <p className="text-[8px] text-slate-400 font-bold uppercase tracking-wider">
                  Avisos mora: {Number(entry.client?.pendingNoticeCount || 0)}
                  {entry.client?.lastPendingNoticeDay === todayStr ? ' · Notificado hoy' : ''}
                </p>
              </div>
              <span className="shrink-0 px-2.5 py-1.5 bg-emerald-50 text-emerald-600 rounded-lg text-[8px] font-black uppercase tracking-widest">
                Ver listado
              </span>
            </button>
          ))}
          {pendingPaymentClients.length === 0 && (
            <p className="text-center py-6 text-slate-400 text-xs italic">No hay clientes con deuda vencida.</p>
          )}
        </div>
      </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
         <Card className="h-fit">
            <div className="flex items-center justify-between mb-6">
               <h4 className="text-sm sm:text-md font-black text-slate-800 dark:text-white uppercase tracking-widest flex items-center gap-2"><Clock size={16} className="text-orange-500" /> Urgente: Cuotas vencidas o sin informar</h4>
               <button onClick={() => navigateToCobros('vencido', { viewMode: 'list' })} className="text-[10px] font-bold text-blue-600 uppercase">Ver Todos</button>
            </div>
            <div className="space-y-3">
               {urgentInstallments.slice(0, 5).map(inst => {
                  const client = clientsById[inst.clientId];
                  ...
                  return (
                     <div key={inst.id} className="flex items-center justify-between p-3 bg-white dark:bg-slate-800 rounded-2xl border border-orange-200 dark:border-orange-900/50 shadow-sm">
                        ...
                        <button onClick={() => handleInstallmentNotification(inst, { force: true })} className="shrink-0 p-2 bg-emerald-50 text-emerald-600 rounded-xl hover:bg-emerald-600 hover:text-white transition-all"><Send size={14} /></button>
                     </div>
                  );
               })}
               {urgentInstallments.length === 0 && <p className="text-center py-6 text-slate-400 text-xs italic">No hay cuotas vencidas o sin informar pago.</p>}
            </div>
         </Card>
         ...
         <Card className="h-fit">
            ...
            <div className="space-y-3">
               {policiesRenewingToday.map(p => {
                  const client = clientsById[p.clientId];
               ...
               {policiesRenewingToday.length === 0 && <p className="text-center py-6 text-slate-400 text-xs italic">No hay renovaciones urgentes hoy.</p>}
            </div>
         </Card>
      </div>
    </div>
  ...
  const renderClients = () => {
    ...
    <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-500 no-print">
      ...
      {clientsScope === 'pending' && (
        <div className="bg-rose-50 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-900/40 rounded-2xl px-4 py-3 flex items-center justify-between">
          <p className="text-[10px] font-black text-rose-600 uppercase tracking-widest">
            Mostrando clientes con pagos pendientes ({pendingPaymentClients.length})
          </p>
          <button onClick={() => setClientsScope('all')} className="text-[9px] font-black uppercase tracking-widest text-blue-600">
            Ver todos
          </button>
        </div>
      )}
      ...
      {viewMode === 'grid' ? (
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
           {filteredClients.map(client => {
             const clientPolicies = policiesByClientId[client.id] || [];
             const pendingEntry = pendingPaymentByClientId[client.id];
             ...
             for (let p of clientPolicies) { if (getCoverageStatus(p).label.includes('SIN COBERTURA')) statusAlert = true; }
              ...
              return (
               <Card key={client.id} onClick={() => { if (isClientSelectionMode) { toggleClientSelection(client.id); } else { setSelectedClient(client); setIsDetailOpen(true); } }} className={`relative group cursor-pointer hover:-translate-y-1 ${statusAlert ? 'border-rose-200 dark:border-rose-900/50' : ''} ${selectedClientIds.includes(client.id) ? 'ring-2 ring-blue-400' : ''}`}>
                 ...
                 {clientsScope === 'pending' && pendingEntry && (
                  <div className="mt-3 pt-3 border-t border-rose-100 dark:border-rose-900/40 space-y-2">
                    <p className="text-[8px] font-black uppercase tracking-widest text-rose-500">
                      Mora: {pendingEntry.maxDaysOverdue} día(s) · Cuotas {pendingEntry.overdueInstallments.length}
                    </p>
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); sendPendingPaymentWhatsApp(pendingEntry, { force: true }); }}
                      className="w-full px-3 py-2 rounded-xl bg-emerald-50 text-emerald-600 text-[9px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all"
                    >
                      Enviar WhatsApp
                    </button>
                  </div>
                             ...
                             <div className="flex flex-col gap-1">
                               ...
                               {clientsScope === 'pending' && pendingEntry && (
                                <button
                                  type="button"
                                  onClick={(e) => { e.stopPropagation(); sendPendingPaymentWhatsApp(pendingEntry, { force: true }); }}
                                  className="mt-2 inline-flex w-fit px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 text-[8px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all"
                                >
                                  Enviar WhatsApp
                                </button>
                               )}
  ...
  const renderCobros = () => (
    <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-500 no-print">
         ...
         <div className="flex flex-col md:flex-row gap-3 items-center justify-between w-full">
            <div className="flex bg-slate-100 dark:bg-slate-900 rounded-xl p-1 w-full md:w-auto overflow-x-auto scrollbar-hide">
               {[
                 ...
                 <button key={value} onClick={() => { setCobrosFilter(value); setCobrosReminderScope(null); }} className={`px-4 py-2 rounded-lg text-[9px] sm:text-[10px] font-black uppercase whitespace-nowrap transition-all ${cobrosFilter === value ? 'bg-white dark:bg-slate-800 shadow-sm text-blue-600' : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}`}>{label}</button>
               ))}
            </div>
      ...
      {cobrosReminderScope !== null && (
        <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/40 rounded-2xl px-4 py-3 flex items-center justify-between gap-3">
          <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">
            Mostrando cuotas para aviso: {reminderLabel(cobrosReminderScope)}
          </p>
          <button onClick={clearCobrosReminderScope} className="text-[9px] font-black uppercase tracking-widest text-slate-500">
            Ver todos
          </button>
        </div>
      )}
              ...
              return (
                <Card key={inst.id} className={`border-l-4 ${inst.computedStatus === 'sin_informar' ? 'border-l-orange-500 bg-orange-50/30 dark:bg-orange-900/10' : inst.status === 'pagado' ? 'border-l-emerald-500' : inst.status === 'vencido' ? 'border-l-rose-500' : 'border-l-blue-500'}`}>
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700/50 flex flex-wrap justify-between items-center gap-2">
                    ...
                    <button onClick={() => handleInstallmentNotification(inst, { force: true })} className="px-2.5 py-1.5 rounded-lg bg-emerald-50 text-emerald-600 text-[8px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all">
                      Notificar
                    </button>
                        ...
                        <tr key={inst.id} className="border-b border-slate-50 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-800/80 transition-colors">
                           ...
                           <td className="p-4">
                              <div className="flex items-center gap-3 flex-wrap">
                                 <Badge status={inst.computedStatus} />
                                 <button onClick={() => handleInstallmentNotification(inst, { force: true })} className="px-3 py-1 rounded-lg bg-emerald-50 text-emerald-600 text-[8px] font-black uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all">
                                    WhatsApp
                                 </button>
                                 <div className="flex gap-1 border border-slate-100 dark:border-slate-700 p-0.5 rounded-lg">
                                    ...
                                    <button onClick={() => updateInstallmentStatus(inst, 'pendiente')} className={`px-2 py-1 rounded text-[8px] font-black uppercase transition-all ${inst.status === 'pendiente' ? 'bg-amber-500 text-white' : 'text-slate-400 hover:text-amber-500'}`}>Pend.</button>
      ...
      <div className="max-w-3xl mx-auto space-y-2 animate-in fade-in duration-500 no-print">
        ...
        >
           <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-2">CBU / Alias / Instrucción de pago</label><input value={agencySettings.paymentInstructions || ''} onChange={e=>setAgencySettings({...agencySettings, paymentInstructions: e.target.value})} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 rounded-xl outline-none font-medium text-xs dark:text-white border border-slate-100 dark:border-slate-700" placeholder="Ej: CBU 000000..., Alias miagencia.seguro" /></div>
              ...
              <div className="space-y-4">
                 ...
                 <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 max-h-72 overflow-y-auto pr-2 scrollbar-hide">
                    {[
                      ...
                      { field: 'vehicleBrand', label: 'Marca Vehículo' },
                      { field: 'vehicleModel', label: 'Modelo Vehículo' },
                      { field: 'vehicleYear', label: 'Año Vehículo' },
  ...
  if (!hasFirebaseConfig) {
    return (
      <div className="min-h-screen bg-slate-50 text-slate-900 flex items-center justify-center p-6">
        <div className="w-full max-w-2xl bg-white border border-rose-100 rounded-3xl p-8 space-y-4 shadow-xl shadow-slate-200/50">
          <h2 className="text-xl font-black uppercase tracking-wide text-rose-600">Configura Firebase para iniciar Se-Flow</h2>
          <p className="text-sm text-slate-500">La app no pudo cargar credenciales. Completa tus variables en <code className="bg-slate-100 px-1 py-0.5 rounded">.env</code> y reinicia.</p>
        ...
        <div className="p-6 lg:p-8 flex flex-col h-full overflow-y-auto scrollbar-hide">
          ...
          <nav className="flex-1 space-y-1 sm:space-y-2">
            {[ { id: 'dashboard', icon: LayoutDashboard, label: 'Inicio' }, { id: 'clients', icon: Users, label: 'Clientes' }, { id: 'vencimientos', icon: CalendarDays, label: 'Cobros' }, { id: 'reports', icon: FileText, label: 'Reportes' }, { id: 'settings', icon: Settings, label: 'Ajustes' } ].filter(item => allowedTabs.includes(item.id)).map(item => (
              <button key={item.id} onClick={() => { setActiveTab(item.id); if (item.id === 'clients') setClientsScope('all'); if (item.id === 'vencimientos') setCobrosReminderScope(null); setSidebarOpen(false); }} className={`w-full flex items-center gap-3 sm:gap-4 px-4 sm:px-5 py-3 sm:py-3.5 rounded-xl sm:rounded-2xl text-xs font-semibold transition-all ${activeTab === item.id ? 'bg-slate-900 dark:bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-slate-800 dark:hover:text-white'}`}>
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-2 sm:p-4 no-print">
          ...
          <div className="relative bg-white dark:bg-slate-900 w-full max-w-3xl rounded-[2rem] sm:rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in slide-in-from-bottom-8 duration-500 flex flex-col max-h-[95vh]">
            <div className="p-5 sm:p-8 border-b border-slate-50 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50 shrink-0">
              ...
              <button onClick={() => { setModalOpen(false); setVehicleBrandForm('Toyota'); }} className="text-slate-300 hover:text-slate-600 p-1.5 sm:p-2"><X size={18} className="sm:w-5 sm:h-5" /></button>
            </div>
            <form ref={policyFormRef} onSubmit={handleCreateOrUpdateClientAndPolicy} className="p-5 sm:p-8 overflow-y-auto flex-1">
              ...
              <div className="mb-6 sm:mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                  ...
                  <div>
                    <label className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase mb-1.5 sm:mb-2 block tracking-widest">Marca</label>
                    <select name="vehicleBrand" value={vehicleBrandForm} onChange={(e) => setVehicleBrandForm(e.target.value)} className="w-full px-4 sm:px-5 py-2.5 sm:py-3.5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl sm:rounded-2xl appearance-none outline-none focus:ring-2 focus:ring-indigo-500/20 dark:text-white text-xs sm:text-sm font-medium">
                      {Object.keys(VEHICLE_BRAND_MODELS).map((brand) => (
                        <option key={brand} value={brand}>{brand}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    <label className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase mb-1.5 sm:mb-2 block tracking-widest">Modelo</label>
                    <select key={`model-${vehicleBrandForm}`} name="vehicleModel" className="w-full px-4 sm:px-5 py-2.5 sm:py-3.5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl sm:rounded-2xl appearance-none outline-none focus:ring-2 focus:ring-indigo-500/20 dark:text-white text-xs sm:text-sm font-medium">
                  ...
                  <div>
                    <label className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase mb-1.5 sm:mb-2 block tracking-widest">Año</label>
                    <input name="vehicleYear" type="number" min="1950" max="2100" placeholder="2022" className="w-full px-4 sm:px-5 py-2.5 sm:py-3.5 bg-slate-50 dark:bg-slate-800 border-none rounded-xl sm:rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500/20 dark:text-white text-xs sm:text-sm font-medium" />
                  </div>
      ...
      {isDetailOpen && selectedClient && (
        <div className="fixed inset-0 z-[110] flex justify-end no-print">
          ...
          <div className="relative bg-[#FDFDFD] dark:bg-slate-900 w-full md:max-w-3xl h-full shadow-2xl overflow-y-auto animate-in slide-in-from-right duration-500 p-6 sm:p-10 flex flex-col">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8 sm:mb-10 shrink-0">
              <Card className="border border-slate-100 dark:border-slate-700/50">
                <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Clasificación del Cliente</p>
                {canManageClaims ? (
                  <select
                    value={selectedClient.riskCategory || 'Bueno'}
                    onChange={(e) => {
                      const nextRisk = e.target.value;
                      setSelectedClient(prev => ({ ...prev, riskCategory: nextRisk }));
                      updateClientRiskCategory(selectedClient.id, nextRisk);
                    }}
                    className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-700 rounded-xl outline-none text-xs sm:text-sm font-bold dark:text-white"
                  >
                    <option value="Bueno">Bueno</option>
                    <option value="Riesgoso">Riesgoso</option>
                  </select>
                ) : (
                  <div className={`inline-flex px-3 py-2 rounded-xl text-xs font-black uppercase tracking-widest ${selectedClient.riskCategory === 'Riesgoso' ? 'bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-300' : 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300'}`}>
                    {selectedClient.riskCategory || 'Bueno'}
                  </div>
                )}
              </Card>
              <Card className="border border-slate-100 dark:border-slate-700/50">
                <p className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Resumen de Siniestros</p>
                <div className="flex items-center justify-between text-xs sm:text-sm">
                  <span className="font-bold text-slate-500 dark:text-slate-300">Cantidad</span>
                  <span className="font-black text-slate-800 dark:text-white">{selectedClientClaimsSummary.total}</span>
                </div>
                <div className="flex items-center justify-between text-xs sm:text-sm mt-2">
                  <span className="font-bold text-slate-500 dark:text-slate-300">Monto total</span>
                  <span className="font-black text-slate-800 dark:text-white">${selectedClientClaimsSummary.totalAmount.toLocaleString('es-AR')}</span>
                </div>
              </Card>
            </div>

            <Card className="mb-8 sm:mb-10 border border-slate-100 dark:border-slate-700/50 shrink-0">
              <div className="flex items-center justify-between mb-4">
                <h4 className="text-[10px] sm:text-xs font-black text-slate-500 uppercase tracking-[0.18em]">Siniestros</h4>
                {!canManageClaims && <span className="text-[9px] font-bold text-slate-400 uppercase">Solo lectura</span>}
              </div>
              {canManageClaims && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                  <div>
                    <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block mb-1.5">Fecha</label>
                    <input type="date" value={claimForm.date} onChange={(e) => setClaimForm(prev => ({ ...prev, date: e.target.value }))} className="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 outline-none text-xs dark:text-white" />
                  </div>
                  <div>
                    <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block mb-1.5">Matrícula Asociada</label>
                    <select value={claimForm.policyId} onChange={(e) => setClaimForm(prev => ({ ...prev, policyId: e.target.value }))} className="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 outline-none text-xs dark:text-white">
                      <option value="">Sin matrícula</option>
                      {selectedClientPolicies.map((p) => (
                        <option key={p.id} value={p.id}>{p.matricula} · {p.vehicleType}</option>
                      ))}
                    </select>
                  </div>
                  <div>
                    <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block mb-1.5">Monto cobertura (opcional)</label>
                    <input type="number" step="0.01" value={claimForm.amount} onChange={(e) => setClaimForm(prev => ({ ...prev, amount: e.target.value }))} placeholder="0.00" className="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 outline-none text-xs dark:text-white" />
                  </div>
                  <div>
                    <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block mb-1.5">Observación</label>
                    <input value={claimForm.notes} onChange={(e) => setClaimForm(prev => ({ ...prev, notes: e.target.value }))} placeholder="Detalle breve del siniestro" className="w-full px-3 py-2.5 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 outline-none text-xs dark:text-white" />
                  </div>
                  <div className="md:col-span-2">
                    <button type="button" onClick={addClientClaim} className="w-full sm:w-auto px-5 py-3 rounded-xl bg-rose-600 text-white text-[10px] font-black uppercase tracking-widest">Agregar Siniestro</button>
                  </div>
                </div>
              )}

              <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                {selectedClientClaims.map((cl) => (
                  <div key={cl.id} className="p-3 rounded-xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/40">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-xs font-black text-slate-800 dark:text-white">{cl.policyMatricula || 'Sin matrícula'} · {cl.date || '-'}</p>
                      <p className="text-xs font-black text-rose-600 dark:text-rose-400">{cl.amount ? `$${Number(cl.amount).toLocaleString('es-AR')}` : 'Monto no informado'}</p>
                    </div>
                    {cl.notes && <p className="text-[10px] text-slate-500 dark:text-slate-300 mt-1">{cl.notes}</p>}
                    <p className="text-[9px] text-slate-400 mt-1">Carga: {cl.createdBy || 'Sistema'}</p>
                  </div>
                ))}
                {selectedClientClaims.length === 0 && <p className="text-xs text-slate-400 italic text-center py-3">Sin siniestros registrados.</p>}
              </div>
            </Card>
               ...
               {selectedClientPolicies.map(p => {
                 ...
                 <Card key={p.id} className={`relative overflow-hidden ${statusBadge.label.includes('SIN COBERTURA') ? 'border-rose-200 dark:border-rose-900' : ''}`}>
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-4 sm:mb-6 gap-4">
                       <div className="flex items-center gap-3 sm:gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">{p.vehicleBrand || 'Marca'} · {p.vehicleModel || 'Modelo'} {p.vehicleYear ? `(${p.vehicleYear})` : ''}</p>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl mb-4">
                       ...
                       <div><p className="text-[9px] font-bold text-slate-400 uppercase">Marca / Modelo / Año</p><p className="text-xs font-semibold dark:text-white">{p.vehicleBrand || '-'} / {p.vehicleModel || '-'} / {p.vehicleYear || '-'}</p></div>
                       <div className="col-span-2 md:col-span-4">
                          <p className="text-[9px] font-bold text-slate-400 uppercase mb-1">Datos Vehículo</p>
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-2">
                            <input
                              type="text"
                              placeholder="Marca"
                              defaultValue={p.vehicleBrand || ''}
                              onBlur={(e) => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'policies', p.id), { vehicleBrand: e.target.value })}
                            ...
                            <input
                              type="text"
                              placeholder="Modelo"
                              defaultValue={p.vehicleModel || ''}
                              onBlur={(e) => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'policies', p.id), { vehicleModel: e.target.value })}
                              className="w-full bg-white dark:bg-slate-800 text-[10px] px-2 py-1.5 rounded border border-slate-200 dark:border-slate-700 outline-none"
                            />
                            <input
                              type="number"
                              ...
                              max="2100"
                              placeholder="Año"
                              defaultValue={p.vehicleYear || ''}
                              onBlur={(e) => updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'policies', p.id), { vehicleYear: e.target.value })}
                              className="w-full bg-white dark:bg-slate-800 text-[10px] px-2 py-1.5 rounded border border-slate-200 dark:border-slate-700 outline-none"
                            />
                          </div>
        ...
        <div ref={printClientSheetRef} className="print-only w-full">
           ...
           <div className="grid grid-cols-2 gap-8 mb-10 bg-slate-50 p-6 rounded-2xl border border-slate-200">
              ...
              <div>
                 ...
                 <p className="text-sm text-slate-900 font-bold mt-2">Riesgo: {selectedClient.riskCategory || 'Bueno'}</p>
                 <p className="text-sm text-slate-900 font-bold mt-1">Siniestros: {selectedClientClaimsSummary.total}</p>
                 <p className="text-sm text-slate-900 font-bold mt-1">Monto histórico siniestros: ${selectedClientClaimsSummary.totalAmount.toLocaleString('es-AR')}</p>
              </div>
              {selectedClient.notes && (
              ...
              {selectedClientPolicies.map(p => {
                ...
                <div key={p.id} className="border-2 border-slate-100 rounded-3xl p-6 break-inside-avoid">
                   ...
                   <div className="grid grid-cols-3 gap-6 mt-4">
                      <div><p className="text-[9px] font-bold text-slate-400 uppercase">Marca</p><p className="text-sm font-semibold">{p.vehicleBrand || '-'}</p></div>
                      <div><p className="text-[9px] font-bold text-slate-400 uppercase">Modelo</p><p className="text-sm font-semibold">{p.vehicleModel || '-'}</p></div>
                      <div><p className="text-[9px] font-bold text-slate-400 uppercase">Año</p><p className="text-sm font-semibold">{p.vehicleYear || '-'}</p></div>
                   </div>
